 ...
 * @date 2020/11/2
 */
public abstract class PageObservable2<T> implements IPageableObservable<T> {
    ...
    public PageObservable2(IPageView<T> view) {
        this(view, 1);
    }

    public PageObservable2(IPageView<T> view, int startIndex) {
    ...
    protected void notifyError(Throwable e) {
        Logger.e(TAG, "分页请求发生错误：" + e.getMessage(), e);
        if (mView == null) {
            return;
        }
